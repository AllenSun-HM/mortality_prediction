...
class ImputationDataset(Dataset):
    ...
    def __init__(self, data, indices, mean_mask_length=3, masking_ratio=0.15,
        ...
        self.missing_masks = self.data.missing_mask
    ...
    def __getitem__(self, ind):
        ...
        mask = noise_mask(X, self.masking_ratio, self.mean_mask_length, self.mode, self.distribution,
                          self.exclude_feats, self.missing_masks)  # (seq_length, feat_dim) boolean array
        missing_mask = self.missing_masks[ind]
        return torch.from_numpy(X), torch.from_numpy(mask), self.IDs[ind], missing_mask
...
def collate_unsuperv(data, max_len=None, mask_compensation=False):
    ...
    batch_size = len(data)
    features, masks, IDs, missing_mask = zip(*data)
    # Stack and pad features and masks (convert 2D to 3D tensors, i.e. add batch dimension)
    lengths = [X.shape[0] for X in features]  # original sequence length for each time series
    if max_len is None:
        max_len = max(lengths)
...
def noise_mask(X, masking_ratio, lm=3, mode='separate', distribution='geometric', exclude_feats=None, missing_mask=None):
    """
        ...
        if mode == 'separate':  # each variable (feature) is independent
            ...
            for m in range(X.shape[1]):  # feature dimension
                if exclude_feats is None or m not in exclude_feats:
                    mask[:, m] = geom_noise_mask_single(X.shape[0], lm, masking_ratio, missing_mask[:, m])  # time dimension
...
def geom_noise_mask_single(L, lm, masking_ratio, missing_mask=None):
    """
